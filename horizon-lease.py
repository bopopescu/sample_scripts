...
def lease_sahara_verify(tenant_id, controllername):
    ...
    db = pymysql.connect(host=controllername, port=3306, user='expostack', passwd='XXXXXX', db='expo_keystone')
    cursor = db.cursor()
    cursor.execute("select d.local_id, e.name, f.name from assignment c, id_mapping d, project e, role f where c.actor_id=d.public_id and d.local_id='cluster.leaseadmin@mydomain.com' and c.target_id=e.id and e.id='" + tenant_id + "' and c.role_id=f.id and f.name='admin'")
    if cursor.rowcount == 0:
       return False
    else:
       return True
