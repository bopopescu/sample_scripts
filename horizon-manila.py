...
def share_create(request, size, name, description, proto, snapshot_id=None,
                 ...
                 is_public=None, availability_zone=None,lease_days=None):
    ...
    lease.create_share_lease_record(controllername, name, lease_days, request.user.username, size)
...
def share_delete(request, share_id):
   ...
   #lease.delete_share_lease_record(controllername, share_id, trueowner)
    return manilaclient(request).shares.delete(share_id)


def share_update(request, share_id, name, description, is_public='', lease_days=None):
...
def share_type_list(request):
    sharetypes = manilaclient(request).share_types.list()
    new_types = []
    tenantname = str(request.user.tenant_name).split("'")[0]
    tenant = tenantname.lower()
    if "prod" in tenant:
        for types in sharetypes:
             if "Prod" in str(types.name).split("'")[0] or "prod" in str(types.name).split("'")[0]:
                 new_types.append(types)
        return new_types
    elif "dev" in tenant:
        for types in sharetypes:
             if "dev" in str(types.name).split("'")[0] or "Dev" in str(types.name).split("'")[0]:
                 new_types.append(types)
        return new_types
    else:
