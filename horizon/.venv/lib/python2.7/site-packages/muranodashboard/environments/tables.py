...
def checkowner(environments):
    db = pymysql.connect(host='controller', port=3306, user='expostack', passwd='BSoniC', db='expo_murano')
    cursor = db.cursor()
    owner="admin";
    sql = "select id from environment where name='" + environments.name + "'"
    cursor.execute(sql)
    resultnew = cursor.fetchall()
    for rows in resultnew:
        owner = rows[0]
    sql = "select id from session  where environment_id='" + owner + "'"
    cursor.execute(sql)
    owner1 = ''
    resultnew = cursor.fetchall()
    for rows in resultnew:
    ...
    if (owner1 == ''):
      ...
      sql = "select description  from environment where id='" + owner + "'"
      cursor.execute(sql)
      resultnew = cursor.fetchall()
      for rows in resultnew:
        owner = rows[0]
    pattern = re.compile("openstackId")
    if pattern.search(owner) is None:
      return ""
    else:
       decoded = json.loads(owner)
       pattern1 = re.compile("gatewayNodes")
       if pattern1.search(owner) is None:
        instance_uuid = decoded['Objects']['services'][0]['instance']['openstackId']
       else:
        instance_uuid = decoded['Objects']['services'][0]['gatewayNodes'][0]['instance']['openstackId']
    if instance_uuid is None:
      return "";
    else:
      db = pymysql.connect(host='controller', port=3306, user='expostack', passwd='BSoniC', db='expo_nova')
      cursor = db.cursor()
      sql = "select hostname from instances where uuid='" + instance_uuid + "'"
      cursor.execute(sql)
      resultnew = cursor.fetchall()
      for rows in resultnew:
        owner = rows[0]
    #    return owner
      sql = "select owner from lease_active_vms where hostname='" + owner + "'"
      cursor.execute(sql)
      resultnew = cursor.fetchall()
      for rows in resultnew:
        owner = rows[0]
...
class OwnerColumn(tables.Column):
    ...
    """
    def get_raw_data(self, environment):
        request = self.table.request
        controllername = nova.get_controllername(request)
        return lease.id_to_ownername(controllername, environment.owner_id)
        #return api.lease.check_volume_owner(controllername, environment.owner_id)
...
class UpdateName(tables.UpdateAction):
    def update_cell(self, request, datum, obj_id, cell_name, new_cell_value):
        try:
            ...
            return False
        return True
class UpdateOwner(tables.UpdateAction):
    def update_cell(self, request, datum, obj_id, cell_name, new_cell_value):
        try:
            #mc = api_utils.muranoclient(request)
            controllername1 = str(base.url_for(request, 'compute')).split(':')
            LOG.warning(_("Anil controllername1 Couldn't update environment. Reason: ") + controllername1)
