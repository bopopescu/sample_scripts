...
from openstack_dashboard.api import lease
...
def volume_create(request, size, name, description, volume_type,
                  ...
                  availability_zone=None, source_volid=None, lease_days=1):
    data = {'name': name,
    ...
    controllername1 = str(base.url_for(request, 'volumev2')).split(':')
    controllername2 = str(controllername1[1]).split('/')
    controllername = controllername2[2]
    lease.create_volume_lease_record(controllername, name, lease_days, request.user.username, size)
...
def volume_delete(request, volume_id):
    ...
    lease.delete_volume_lease_record(controllername, volume_id, trueowner)
...
def volume_update(request, volume_id, name, description, lease_days):
    vol_data = {'name': name,
                'description': description}
    ...
    controllername2 = str(controllername1[1]).split('/')
    controllername = controllername2[2]
    lease.update_volume_lease(controllername, volume_id, lease_days)
...
def volume_type_list(request):
    volumetypes = cinderclient(request).volume_types.list()
    new_types = []
    tenantname = str(request.user.tenant_name).split("'")[0]
    tenant = tenantname.lower()
    if "prod" in tenant:
        for types in volumetypes:
             if "Prod" in str(types.name).split("'")[0] or "prod" in str(types.name).split("'")[0] or "Tier2" in str(types.name).split("'")[0]:
                 new_types.append(types)
        return new_types
    elif "dev" in tenant:
        for types in volumetypes:
             if "dev" in str(types.name).split("'")[0] or "Dev" in str(types.name).split("'")[0] or "Tier2" in str(types.name).split("'")[0] or "Tier3" in str(types.name).split("'")[0]:
                 new_types.append(types)
        return new_types
    else:
