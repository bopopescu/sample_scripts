...
def notify_sahara(receiver, vmname, whattodo,ipaddressa,submitter):
    ...
    if whattodo == "createrequeststart":
       msg['Subject'] = "Your request for Sahara Cluster " + vmname + "" + " is submitted"
       html1 = "<html><head></head><body>" + font + "<b>Cluster Name  : </b>" + vmname + "<br><br> <b>Thanks and Regards</b><br>Expostack Administrator"

    if whattodo == "deleterequeststart":
       msg['Subject'] = "Your request for Sahara Cluster " + vmname + "" + " destroy has been submitted"
       html1 = "<html><head></head><body>" + font + "<b>Cluster Name  : </b>" + vmname + "<br><b>Submitted By</b> :" + submitter + "<br><b> Thanks and Regards</b><br>Expostack Administrator"
...
def lease_sahara_verify(tenant_id, controllername):

    db = pymysql.connect(host=controllername, port=3306, user='expostack', passwd='BSoniC', db='expo_keystone')
    cursor = db.cursor()
    cursor.execute("select d.local_id, e.name, f.name from assignment c, id_mapping d, project e, role f where c.actor_id=d.public_id and d.local_id='cluster.leaseadmin@exponential.com' and c.target_id=e.id and e.id='" + tenant_id + "' and c.role_id=f.id and f.name='admin'")
    if cursor.rowcount == 0:
       return False
    else:
       return True
...
def check_cluster_owner(controllername, cluster_name):
    db = pymysql.connect(host=controllername, port=3306, user='expostack', passwd='BSoniC', db='expo_sahara')
    cursor = db.cursor()
    sql = "select owner from leaseinfo where cluster_name='" + cluster_name + "'"
    cursor.execute(sql)
    resultnew = cursor.fetchall()
    for rows in resultnew:
        owner = rows[0]
...
def check_volume_owner(controllername, instancename):
    db = pymysql.connect(host=controllername, port=3306, user='expostack', passwd='BSoniC', db='expo_cinder')
    cursor = db.cursor()
...
def create_sahara_lease_record(controllername ,clustername, lease_days, owner):
    ...
    if cursor.rowcount == 0:
       ...
       if cursor.rowcount == 0:
	  sql = "insert into leaseinfo values ('" + clustername + "','" + lease_days_pass + "','" + owner + "')";
       else:
       ...
       db.commit()
       db.close()
       notify_sahara(owner,clustername,"createrequeststart","nil","nil")
...
def create_share_lease_record(controllername ,instancename, lease_days, owner, size):
    db = pymysql.connect(host=controllername, port=3306, user='expostack', passwd='BSoniC', db='expo_manila')
    cursor = db.cursor()
    lease_days_pass = str(lease_days)
    generatetime1 = datetime.strftime(datetime.now(), '%Y-%m-%d %H:%M:%S')
    generatetime = str(generatetime1)
...
def check_share_owner(controllername, instancename):
    db = pymysql.connect(host=controllername, port=3306, user='expostack', passwd='BSoniC', db='expo_manila')
    cursor = db.cursor()
    sql = "select owner from lease_active_shares where shareuuid='" + instancename + "'"
    cursor.execute(sql)
    resultnew = cursor.fetchall()
    owner = ""
    for rows in resultnew:
...
def check_sahara_owner(controllername, instancename):
    db = pymysql.connect(host=controllername, port=3306, user='expostack', passwd='BSoniC', db='expo_sahara')
    cursor = db.cursor()
    sql = "select owner from lease_active_clusters where Cluster_id='" + instancename + "'"
    cursor.execute(sql)
    resultnew = cursor.fetchall()
    owner = ""
    for rows in resultnew:
...
def check_share_lease(controllername, instancename , instancetenantid):
    db = pymysql.connect(host=controllername, port=3306, user='expostack', passwd='BSoniC', db='expo_manila')
    cursor = db.cursor()
    date_formatv='%Y-%m-%d %H:%M:%S'
    datev = datetime.now(tz=pytz.utc)
    currentdatev = datetime.strftime(datev, date_formatv);
    sql = "select case requesttype when 'days' then DATE_ADD(created_on,INTERVAL leasedays DAY) when 'hours' then DATE_ADD(created_on,INTERVAL leasedays HOUR) end as leasehowmuch ,case requesttype when 'days' then timestampdiff(MINUTE,'"+ currentdatev +"',DATE_ADD(created_on,INTERVAL leasedays DAY)) " +                   "when 'hours'  then timestampdiff(MINUTE,'"+ currentdatev +"',DATE_ADD(created_on,INTERVAL leasedays HOUR)) end as jordan from lease_active_shares where shareuuid='" + instancename + "'"
    cursor.execute(sql)
    resultnew = cursor.fetchall()
    LOG.error("This is instancetenantid")
    LOG.error(instancetenantid)
    for rows in resultnew:
        if rows[1] < 0:
           leaseexpire = "Expired";
        else:
...
def check_sahara_lease(controllername, instancename , instancetenantid):
    db = pymysql.connect(host=controllername, port=3306, user='expostack', passwd='BSoniC', db='expo_sahara')
    cursor = db.cursor()
    date_formatv='%Y-%m-%d %H:%M:%S'
    datev = datetime.now(tz=pytz.utc)
    currentdatev = datetime.strftime(datev, date_formatv);
    sql = "select case requesttype when 'days' then DATE_ADD(created_on,INTERVAL leasedays DAY) when 'hours' then DATE_ADD(created_on,INTERVAL leasedays HOUR) end as leasehowmuch ,case requesttype when 'days' then timestampdiff(MINUTE,'"+ currentdatev +"',DATE_ADD(created_on,INTERVAL leasedays DAY)) " +                   "when 'hours'  then timestampdiff(MINUTE,'"+ currentdatev +"',DATE_ADD(created_on,INTERVAL leasedays HOUR)) end as jordan from lease_active_clusters where Cluster_id='" + instancename + "'"
    cursor.execute(sql)
    LOG.error(sql)
    resultnew = cursor.fetchall()
    for rows in resultnew:
        if rows[1] < 0:
           ...
           leaseexpire = rows[0]
        if lease_sahara_verify(instancetenantid, controllername) == False:
           leaseexpire = "Never"
        return leaseexpire
