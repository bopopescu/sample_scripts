...
def server_create(request, name, image, flavor, key_name, user_data,
                  ...
                  disk_config=None, config_drive=None, meta=None,lease_days=1,flavor_name=None):
    ...
    flavor_name = novaclient(request).flavors.get(flavor)
    lease_days_pass = str(lease_days)
    lease.create_lease_record(controllername, name, lease_days_pass, request.user.username, flavor_name.name)
...
def server_delete(request, instance):
    ...
    lease.delete_lease_record(controllername, instance, trueowner)
...
#Following function was added in reference to GIT-1298, this function filters server list as per user.
def server_user_list(request, search_opts=None, all_tenants=False):
    page_size = utils.get_page_size(request)
    c = novaclient(request)
    compare_username = request.user.id
    paginate = False
    ...
    if all_tenants:
        ...
        search_opts['user_id'] = request.user.id
        #compare_username = request.user.id
        #LOG.error("Project id our = %s " % search_opts['project_id'])
        #LOG.error("user id our = %s " % search_opts['user_id'])

    servers = [Server(s, request)
               for s in c.servers.list(True, search_opts)]


    #LOG.error("Server list: %s" % servers)


    user_list = []

    for user in servers:
        if user.user_id == compare_username:
    ...
    return (user_list, has_more_data)
