...
def notify_sahara_cluster(receiver, vmname, whattodo,ipaddressa,submitter):
    me = "expostack@exponential.com"
    msg = MIMEMultipart('related')
    msgAlternative = MIMEMultipart('alternative')
    receiversplit = receiver.split("@")
    ...
    if whattodo == "createrequeststart":
       ...
       html1 = "<html><head></head><body>" + font + "<b>Cluster Name  : </b>" + vmname + "<br><b>Flavor : </b><br><br> <b>Thanks and Regards</b><br>Expostack Administrator"
...
def create_sahara_lease_record(controllername ,clustername, lease_days, owner):
    ...
    if cursor.rowcount == 0:
       ...
       if cursor.rowcount == 0:
          ...
          sql = "update leaseinfo set leasedays='" + lease_days_pass + "' where vmname='" + clustername + "'"

       cursor.execute(sql)
       db.commit()
       db.close()
       notify_sahara_cluster(owner,instancename,"createrequeststart","nil","nil")
