...
def cluster_create(request, name, plugin_name, hadoop_version,
                   ...
                   net_id=None, count=None, use_autoconfig=None, lease_days=1):
    ...
    #LOG.error("This is controller name")
    #LOG.error(controllername)
    #LOG.error(request)

    lease.create_sahara_lease_record(controllername, name, lease_days_pass, request.user.username)
...
def cluster_list(request, search_opts=None):
    #LOG.error("Following is Search opts **********************************")
