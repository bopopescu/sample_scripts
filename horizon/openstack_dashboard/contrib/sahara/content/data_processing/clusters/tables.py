...
class OwnerColumn(tables.Column):
     ...
     for a share instance.
     """
     def get_raw_data(self, cluster):
         request = self.table.request
         controllername = api.nova.get_controllername(request)
         return api.lease.check_sahara_owner(controllername, cluster.id)
...
class LeaseColumn(tables.Column):
     ...
     def get_raw_data(self, cluster):
         request = self.table.request
         controllername = api.nova.get_controllername(request)
         #LOG.error(cluster.id)
	 #LOG.error(cluster.tenant_id)
         return api.lease.check_sahara_lease(controllername, cluster.id, cluster.tenant_id)
