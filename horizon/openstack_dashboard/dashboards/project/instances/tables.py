...
def checkowner(instance):
    for jk in instance.links:
        controllername1 = str(jk['href']).split(':')
        controllername2 = str(controllername1[1]).split('/')
        controllername = controllername2[2]
    LOG.info("Rocking : %s" % str(controllername))
    return api.lease.check_instance_owner(controllername, instance.name)
...
def checkowner2(instance):
	...
	username = user.name
        ...
        LOG.error("Check owner2 checkowner2 exception fired /home/horizon/horizon/openstack_dashboard/dashboards/project/instances/tables.py")
	username = "User account unavailable"
...
def checklease(instance):
    for jk in instance.links:
        ...
        controllername2 = str(controllername1[1]).split('/')
        controllername = controllername2[2]
    return api.lease.check_instance_lease(controllername, instance.name, instance.tenant_id)
...
class InstancesTable(tables.DataTable):
    ...
    ownern = tables.Column(checkowner2,verbose_name=_("Owner ID"))
