...
class UpdateInstanceInfoAction(workflows.Action):
    name = forms.CharField(label=_("Name"),
                           max_length=255,widget=forms.TextInput(attrs={'readonly':'readonly'}))
...
class UpdateInstanceInfo(workflows.Step):
    ...
    depends_on = ("instance_id",)
    contributes = ("name","lease_days",)

class UpdateInstanceInfoActionAdmin(workflows.Action):
    name = forms.CharField(label=_("Name"),
                           max_length=255,widget=forms.TextInput(attrs={'readonly':'readonly'}))
...
class UpdateInstanceInfoAdmin(workflows.Step):
    action_class = UpdateInstanceInfoActionAdmin
    depends_on = ("instance_id",)
    contributes = ("name","lease_days",)
...
class AdminUpdateInstance(UpdateInstance):
    ...
    default_steps = (UpdateInstanceInfoAdmin,)
