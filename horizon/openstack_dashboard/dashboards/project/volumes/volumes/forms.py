...
# under the License.
# Comment: File has hardcoded values for number of volumes per user and size of volume
...
class CreateForm(forms.SelfHandlingForm):
    ...
    lease_days = forms.IntegerField(label=_("Lease Days"),
                               min_value=1,
                               max_value=180,
                               initial=31,
                               help_text=_("Number of Lease Days. (Max:180days)"))    
    ...
    def handle(self, request, data):
        try:
            ...
            user_vol_total = cinder.volume_list(request)

            if len(user_vol_total) >= USER_QUOTA:
                msg = _('As per Expostack policy, only %s volumes are allowed per user in a perticular Project.'
                        'Please contact NOC@exponential.com for any queries.') % USER_QUOTA
                raise ValidationError(msg)
    
            if data['size'] > VOLUME_SIZE_LIMIT:
            	msg = _('As per Expostack policy Volume size cannot be more than %s GB.'
                        'Please contact NOC@exponential.com for any queries.') % VOLUME_SIZE_LIMIT
                raise ValidationError(msg)
            ...
            volume = cinder.volume_create(request,
                                          ...
                                          source_volid=volume_id, lease_days=data['lease_days'])
        ...
        try:
            ...
            try:
                if len(user_snap_total) >= USER_QUOTA:
                    message = _('As per Expostack policy, only %s snapshots are allowed per user in a perticular project.'
                        'Please contact NOC@exponential.com for any queries.') % USER_QUOTA
...
class UpdateForm(forms.SelfHandlingForm):
    ...
    description = forms.CharField(max_length=255,
                                  ...
                                  label=_("Description"),
                                  required=False)
    lease_days = forms.IntegerField(label=_("Increase Lease Days By"),
                               min_value=1,
                               max_value=180,
                               initial=31,
                               help_text=_("Number of Lease Days. (Max:180days)"))
    ...
    def handle(self, request, data):
        ...
        try:
            cinder.volume_update(request, volume_id, data['name'],
                                 data['description'], data['lease_days'])
        except Exception:
            redirect = reverse("horizon:project:volumes:index")
            exceptions.handle(request,
...
class ExtendForm(forms.SelfHandlingForm):
    ...
    def clean(self):
        ...
        availableGB = usages['maxTotalVolumeGigabytes'] - \
            usages['gigabytesUsed']
        if availableGB < (new_size - orig_size) or new_size > VOLUME_SIZE_LIMIT:
            message = _('Volume cannot be extended to %(req)iGB, as per Expostack policy total volume size should be less than %s GB.'
                        'Please contact NOC@exponential.com for any queries.') % VOLUME_SIZE_LIMIT
