...
class LeaseColumn(tables.Column):
    """Customized column class.

    So it that does complex processing on the Lease
    ...
    def get_raw_data(self, volume):
        ...
        project_id = getattr(volume, "os-vol-tenant-attr:tenant_id", None)
        return api.lease.check_volume_lease(controllername, volume.id, project_id)

class OwnerColumn(tables.Column):
    """Customized column class.

    So it that does complex processing on the Owner
    for a volume instance.
    """
    def get_raw_data(self, volume):
        request = self.table.request
        controllername = api.nova.get_controllername(request)
        project_id = getattr(volume, "os-vol-tenant-attr:tenant_id", None)
        return api.lease.check_volume_owner(controllername, volume.id)
...
class VolumesTableBase(tables.DataTable):
    ...
    #ownern = tables.Column("user_id",verbose_name=_("Owner ID"))
...
def checkowner2(self):
    LOG.error(vars(self))
    LOG.error(dir(self))
    LOG.error(type(self))
    LOG.error(self._attrs)
    LOG.error(self.__class__)
    LOG.error(self.__delattr__)
    LOG.error(self.__dict__)
    LOG.error(self.__doc__)
    LOG.error(self.__format__)
    LOG.error(self.__getattribute__)
    LOG.error(self.__hash__)
    LOG.error(self.__init__)
    LOG.error(self.__module__)
    LOG.error(self.__new__)
    LOG.error(self.__reduce__)
    LOG.error(self.__reduce_ex__)
    LOG.error(self.__repr__)
    LOG.error(self.__setattr__)
    LOG.error(self.__sizeof__)
    LOG.error(self.__str__)
    LOG.error(self.__subclasshook__)
    LOG.error(self.__weakref__)
    LOG.error(self._apiresource)
    LOG.error(self._attrs)
    LOG.error(self.description)
    LOG.error(self.is_bootable)
    LOG.error(self.name)
    LOG.error(self.to_dict)
    #LOG.error(instance.request)
    #LOG.error(volume.metadata)
    #LOG.error(volume.user_id)
    #request = self.table.request
    #LOG.error(vars(request.to_dict))
    #LOG.error(request.to_dict)
...
class VolumesTable(VolumesTableBase):
    ...
    volume_type = tables.Column(get_volume_type,
                                verbose_name=_("Type"))
    lease_days = LeaseColumn("leasedays",
                                   verbose_name=_("Expire on (UTC)"))
    owner_name = OwnerColumn("ownername",
