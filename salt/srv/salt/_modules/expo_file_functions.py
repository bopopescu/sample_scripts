...
def comment_line(path,
    ...
    ret = {'pid': rand,
           'retcode': rcode,
           'stderr': '',
           'stdout': ''.join(difflib.unified_diff(orig_file, new_file))}
    return ret
...
def append(path, *args, **kwargs):
    ...
    ret = {'pid': rand,
           'retcode': rcode,
           'stderr': '',
           'stdout': 'Wrote {0} lines to "{1}"'.format(len(args), path)}
    return ret
...
def symlink(src, path):
    ...
    try:
        ...
        ret = {'pid': rand,
               'retcode': fcode,
               'stderr': ' Could not create Symlink.',
               'stdout': ' Could not create \'{0}\''.format(path)}
        return ret
...
def access(path, mode):
    ...
    if not os.path.isabs(path):
        ret = {'pid': rand,
               'retcode': fcode,
               'stderr': ' Path to link must be absolute.',
               'stdout': ' Could not find the path!'}
        return ret
    modes = {'f': os.F_OK,
             'r': os.R_OK,
             'w': os.W_OK,
    ...
    if mode in modes:
        ret = {'pid': rand,
               'retcode': rcode,
               'stderr': '',
               'stdout': ' User has access to the file'}
        return ret
    elif mode in six.itervalues(modes):
        ret = {'pid': rand,
               'retcode': fcode,
               'stderr': ' User has no access to the file',
               'stdout': ' File access check Unsuccessful.'}
        return ret
    else:
        ret = {'pid': rand,
               'retcode': fcode,
               'stderr': ' Mode specified is Invalid and does not exists.',
               'stdout': ' Mode is Invalid.'}
        return ret
...
def rmdir(path):
    ...
    try:
        ...
        ret = {'pid': rand,
               'retcode': fcode,
               'stderr': ' Could not remove directory.',
               'stdout': ' Remove Failed.'}
        return ret
...
def remove(path):
    ...
    ret = {'pid': rand,
           'retcode': fcode,
           'stderr': ' Could not remove file.',
           'stdout': 'Could not remove \'{0}\': {1}'.format(path, exc)}
    return ret
