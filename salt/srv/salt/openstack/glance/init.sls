...
{%- set messagingUser = salt['pillar.get']('openstack:MESSAGING_USER', messagingUser) %}
{%- set messagingPass = salt['pillar.get']('openstack:MESSAGING_PASS', messagingPass) %}
...
{{fileName}} - Create an API endpoint for the Image Service:
  keystone.endpoint_present:
    - name: "glance"
    - publicurl: "http://{{controllerHost}}:9292"
    - internalurl: "http://{{controllerHost}}:9292"
    - adminurl: "http://{{controllerHost}}:9292"
    - connection_token: "{{adminToken}}"
    - connection_endpoint: "{{endpointURL}}"
    - require:
      - pkg: {{fileName}} - Install OpenStack Image Service
...
{{fileName}} - Upload {{imageName}} to the Image service:
  cmd.run:
    - name: 'glance --os-username={{adminUser}} --os-password="{{adminPass}}" --os-tenant-name={{adminUser}} --os-auth-url="{{endpointURL}}" image-create --name "{{imageName}}" --disk-format {{diskFormat}} --container-format bare --visibility public < /tmp/images/{{imageName}}.{{imageExtension}}'
    - unless: 'glance --os-username={{adminUser}} --os-password="{{adminPass}}" --os-tenant-name={{adminUser}} --os-auth-url="{{endpointURL}}" image-show "{{imageName}}"'
    - require:
